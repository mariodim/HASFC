...
def validate_post_input(data):
    if 'Costs' in data and data['Costs'] is not None and len(data['Costs']) == 3:
        try: 
            ...
            raise ValidationException('Costs should be an array of float. Try to use default [1,1,1]')
    else:
        raise ValidationException('Costs should be an array of size 3. Try to use default [1,1,1]')
    ...
    if 'AvailabilityTarget' in data and data['AvailabilityTarget'] is not None:
        if type(data['AvailabilityTarget']) == str:
            try:
                ...
                raise ValidationException('AvailabilityTarget should be an array of float. Try to use default [0.9999,0.99999,0.999999]')
        ...
        if 'maxVNF' in paramsJSON and paramsJSON['maxVNF'] is not None:
            try:
                ...
                if params['maxVNF'] < 0 and params['maxVNF'] > 7:
                    raise ValidationException('maxVNF should be an integer less then 7')
            except ValueError:
                raise ValidationException('maxVNF should be an integer less than 7')
        else:
            raise ValidationException('maxVNF should be an integer less than 7')
