...
def run_timenet(parameters, id):
    try:
        ...
        for subset in itertools.combinations_with_replacement(
                [k for k in range(max_VNF, -1, -1)], int(max_NR)):
            values.add(subset)
        ...
        for perf in range(3, 5):
            for value in values:
                ...
                if VNF1 + VNF2 + VNF3 + VNF4 > perf:
                    ...
                    input_file_to_timenet = open(
                        f"homogeneous\\data_{id}", 'w')
                    ...
                    try:
                        ...
                        subprocess.run(
                            '"C:\\Program Files (x86)\\TimeNET\\TimeNET\\EDSPN\\StatAnalysis\\scripts\\SOLVE.bat" ' +
                            id +
                            ' -E -s -i 10000 1e-10',
                        ...
                        output_file.write(
                            f"{VNF1};{VNF2};{VNF3};{VNF4};{perf};{result}\n")
...
def cost_calculator(configuration, costs=[0.5, 0.5, 0.5]):
    ...
    for i in range(0, 7, 2):
        if int(configuration[i]) != 0:
            cost = cost + costs[1] + costs[2] + \
                costs[0] * int(configuration[i])
    return cost
