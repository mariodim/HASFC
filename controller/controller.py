...
def availability_post(data):
    ...
    if id is not None:
        return {
            'Status': 'Success',
            'Message': 'Results available in cache. Use the Id to access.',
            'Id': id}
    ...
    return {
        'Status': 'Success',
        'Message': 'Results will be available as soon as possible. Use the Id to access.',
        'Id': id}
...
def availability_get(data):
    ...
    try:
        with open("logs//" + id + "_log", 'r') as logfile:
            ...
            if perc == 100:
                out_filename = "results//" + id + "_" + \
                    str(availability_target) + "_results.csv"
                ...
                return {
                    ...
                    'Message': str(
                        round(
                            ...
                            3)) +
                    ' % ' +
                    'results ready. We will finish as soon as possible',
                    'Id': id}
    except Exception:
